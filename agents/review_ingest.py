...
Handles review ingestion from multiple formats:
- CSV files (ingest_csv)
- JSON data (ingest_json)
- Raw pasted text (ingest_text) — the demo hero

All methods normalize to a standard schema and save to:
  data/businesses/{business_id}/reviews.json
"""
import os
import json
import csv
...
def _save_reviews(business_id: str, reviews: list) -> str:
    """Saves normalized reviews to disk. Appends to existing if present."""
...
def _normalize_review(raw: dict, source: str = "unknown") -> dict:
    """Normalizes a raw review dict to the standard schema."""
    return {
...
def ingest_csv(file_path: str, business_id: str) -> list:
    ...
    Flexibly handles column names: text/review_text/comment, rating/stars/score, etc.
    """
    reviews = []
...
def ingest_text(raw_text: str, business_id: str) -> list:
    """
    The demo hero. Uses Cerebras to parse messy pasted text into structured reviews.

    The business owner can paste a wall of review text from any source —
    Google reviews, Yelp screenshots, handwritten notes — and this function
    extracts individual reviews with ratings.

    Args:
        raw_text: The raw pasted text containing one or more reviews.
        business_id: The business identifier.

    Returns:
        A list of normalized review dicts.
    """
    prompt = f"""You are a review parser. The user pasted raw review text from their business.
...
def get_review_count(business_id: str) -> int:
    """Returns the number of reviews stored for a business."""
    return len(load_reviews(business_id))


def list_businesses() -> list:
    """Lists all business IDs with stored data."""
